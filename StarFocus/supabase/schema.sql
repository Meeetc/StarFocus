...
CREATE POLICY "Users can insert own data" ON users FOR INSERT WITH CHECK (auth.uid()::TEXT = auth_id);
...
CREATE TABLE IF NOT EXISTS tasks (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id TEXT NOT NULL, -- stores auth.uid() directly
  classroom_id TEXT,
  course_id TEXT,
  course_name TEXT,
  title TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS streaks (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id TEXT UNIQUE NOT NULL, -- stores auth.uid() directly
...
CREATE TABLE IF NOT EXISTS study_groups (
  ...
  created_by TEXT NOT NULL, -- stores auth.uid() directly
  created_at TIMESTAMPTZ DEFAULT NOW()
);

ALTER TABLE study_groups ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Anyone can view groups" ON study_groups FOR SELECT USING (TRUE);
CREATE POLICY "Authenticated users can create groups" ON study_groups FOR INSERT WITH CHECK (
  created_by = auth.uid()::TEXT
);
CREATE POLICY "Creators manage groups" ON study_groups FOR UPDATE USING (
  created_by = auth.uid()::TEXT
);
CREATE POLICY "Creators delete groups" ON study_groups FOR DELETE USING (
  created_by = auth.uid()::TEXT
);
...
CREATE POLICY "Members view group members" ON group_members FOR SELECT USING (
  group_id IN (SELECT group_id FROM group_members WHERE user_id = auth.uid()::TEXT)
);
...
CREATE POLICY "Public leaderboard read" ON leaderboard_snapshots FOR SELECT USING (TRUE);
